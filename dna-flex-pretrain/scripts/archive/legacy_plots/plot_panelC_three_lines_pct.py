...
@torch.no_grad()
def extract_features(model, ds, indices, use_flex=False, device="cpu"):
...
def main():
    ...
    for seed in args.seeds:
        ...
        Xtr_h, ytr_h = extract_features(model, ds, idx_train, use_flex=False)
        Xva_h, yva_h = extract_features(model, ds, idx_val, use_flex=False)
        Xte_h, yte_h = extract_features(model, ds, idx_test, use_flex=False)

        Xtr_hf, ytr_hf = extract_features(model, ds, idx_train, use_flex=True)
        Xva_hf, yva_hf = extract_features(model, ds, idx_val, use_flex=True)
        Xte_hf, yte_hf = extract_features(model, ds, idx_test, use_flex=True)
        ...
        for frac in args.train_fracs:
            ...
            # standardize transformer features
            ...
            print(
                f"{args.dataset} | seed={seed} | pct={frac*100:.1f}% | n={n_use} | "
                f"1mer={r2_1mer:.3f} | hidden={r2_hidden:.3f} | hidden+flex={r2_hiddenflex:.3f}"
            )

    # save csv
    out_csv = f"plots/panelC_three_lines_pct_{args.dataset}.csv"
    ...
    unique_pct = sorted({r["train_pct"] for r in results})
    mean_1mer, std_1mer = [], []
    mean_hidden, std_hidden = [], []
    mean_hf, std_hf = [], []

    for pct in unique_pct:
        rows_pct = [r for r in results if r["train_pct"] == pct]
        v1 = np.array([r["r2_1mer"] for r in rows_pct])
        v2 = np.array([r["r2_hidden"] for r in rows_pct])
        v3 = np.array([r["r2_hiddenflex"] for r in rows_pct])
    ...
    ax.errorbar(unique_pct, mean_1mer, yerr=std_1mer, marker="o", capsize=4, linewidth=2, label="1-mer ridge")
    ax.errorbar(unique_pct, mean_hidden, yerr=std_hidden, marker="o", capsize=4, linewidth=2, label="Transformer hidden + ridge")
    ax.errorbar(unique_pct, mean_hf, yerr=std_hf, marker="o", capsize=4, linewidth=2, label="Transformer hidden+flex + ridge")

    ax.set_ylim(0.0, 1.0)
    ax.set_xlim(min(unique_pct)-3, max(unique_pct)+3)
    ax.set_xticks(unique_pct)
    ax.set_xticklabels([f"{pct:.1f}%" if pct < 100 else "100%" for pct in unique_pct])

    ax.set_xlabel("% of training split used", fontsize=13)
    ax.set_ylabel("Test $R^2$", fontsize=13)
    ax.set_title(f"Panel C: % training data used vs $R^2$ ({args.dataset}, mean ± std across seeds)", fontsize=14)
    ax.grid(True, alpha=0.25)
    ax.legend()

    out_png = f"plots/panelC_three_lines_pct_{args.dataset}.png"
    fig.tight_layout()
