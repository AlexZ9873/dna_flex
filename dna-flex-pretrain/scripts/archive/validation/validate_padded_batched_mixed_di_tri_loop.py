...
def main():
    ...
    for ex in examples:
        ...
        x_pad = pad_2d(ex["x"], max_tokens, pad_value=0.0)
        labels_pad = pad_1d(ex["mlm_labels"], max_tokens, pad_value=-100)
        flex_pad = pad_2d(ex["flex_targets"], max_tokens, pad_value=0.0)
    ...
    attention_mask_batch = torch.stack(batch_attention_mask, 0)
    mlm_labels_batch = torch.stack(batch_mlm_labels, 0)
    flex_targets_batch = torch.stack(batch_flex_targets, 0)
    flex_valid_batch = torch.stack(batch_flex_valid, 0)
    ...
    print("padded batched mixed di+tri loop complete")
