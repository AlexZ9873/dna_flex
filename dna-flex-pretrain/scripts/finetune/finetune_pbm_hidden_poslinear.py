...
def pbm_collate_pad(batch: List[Dict], max_tokens: int) -> Dict[str, torch.Tensor]:
    """
    Pads variable-length token sequences to max_tokens so we can use a fixed-size head.
    """
    B = len(batch)
...
@torch.no_grad()
def eval_hidden_poslinear(model, head, loader, device) -> Tuple[float, float, float]:
    model.eval()
    head.eval()
    ...
    for batch in loader:
        ...
        # get hidden states
        _, _, h = model(x, am, return_hidden=True)   # h: [B,T,d_model]

        # zero out padding positions before flattening
        h = h * am.unsqueeze(-1).float()

        # position-aware linear head: flatten [B, T, d_model] -> [B, T*d_model]
        B, T, D = h.shape
        feat = h.reshape(B, T * D)
...
def main():
    ...
    # Determine max token length for fixed head input size
    # PBMDataset stores (seq, score) in ds.items. :contentReference[oaicite:2]{index=2}
    max_tokens = 0
    ...
    # Load pretrained model and freeze encoder
    # model.forward(..., return_hidden=True) returns hidden states h. :contentReference[oaicite:3]{index=3}
    ...
    # Position-aware head (like ridge baseline, but on hidden states)
    in_dim = max_tokens * d_model

    # Linear head is closest to ridge; you can swap to MLP later if needed.
    head = nn.Linear(in_dim, 1).to(device)

    lr = float(cfg_ft["train"]["lr"])
    weight_decay = float(cfg_ft["train"].get("weight_decay", 1e-2))  # ridge-like
    ...
    print("PBM fine-tune (FROZEN encoder, HIDDEN states, POSITION-AWARE LINEAR head) starting")
    ...
    print("k =", k, "max_tokens =", max_tokens, "d_model =", d_model, "head_in_dim =", in_dim)
    ...
    for ep in range(1, epochs + 1):
        ...
        val_rmse, val_p, val_r2 = eval_hidden_poslinear(model, head, val_loader, device)
    ...
    test_rmse, test_p, test_r2 = eval_hidden_poslinear(model, head, test_loader, device)
    ...
    out_path = "checkpoints/pbm_hidden_poslinear_best.pt"
    torch.save(
        {
