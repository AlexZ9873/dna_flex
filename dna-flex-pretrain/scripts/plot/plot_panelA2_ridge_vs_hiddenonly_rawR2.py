...
def eval_hiddenonly(model, head, loader, device):
    ...
    for batch in loader:
        ...
        # IMPORTANT: get hidden states directly
...
def main():
    ...
    for ds_name in datasets:
        ...
        for seed in seeds:
            ...
            hidden_res = train_hiddenonly(ds, idx_train, idx_val, idx_test, model, cfg_pre, cfg_ft, seed)
            ...
            print(
                ...
                f"hiddenonly_r2={hidden_res['test_r2']:.3f}"
            )

    # save CSV
    out_csv = "plots/panelA2_ridge_vs_hiddenonly_rawR2.csv"
    ...
    ys = [r["hidden_test_r2"] for r in rows]
    ...
    ax.set_ylabel("Test $R^2$ (hidden-only: frozen encoder + flatten head)", fontsize=13)
    ax.set_title("Panel A2: 1-mer ridge vs hidden-only (raw test $R^2$)", fontsize=14)
    ...
    out_png = "plots/panelA2_ridge_vs_hiddenonly_rawR2.png"
