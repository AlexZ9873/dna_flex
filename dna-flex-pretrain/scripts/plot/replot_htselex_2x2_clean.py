...
def main():
    ...
    parser.add_argument("--out_png", type=str, required=True, help="output PNG")
    parser.add_argument("--figure_title", type=str, required=True, help="whole figure title")
    parser.add_argument("--ycol", type=str, default=None, help="optional explicit y-column")
    parser.add_argument("--dpi", type=int, default=220)
    args = parser.parse_args()

    df = pd.read_csv(args.csv)

    # optional family column for coloring
    ...
    # y-axis column
    if args.ycol is not None:
        if args.ycol not in df.columns:
            raise ValueError(
                "Specified ycol not found: "
                + args.ycol
                + "\nAvailable columns are:\n"
                + ", ".join(df.columns)
            )
        ...
        col_model = resolve_col(df, [
            ...
            "cv_r2_hiddenflex",
            "r2_hiddenflex",
            "cv_r2_transformer_hiddenflex_bend",
            "r2_transformer_hiddenflex_bend",
            "cv_r2_transformer_hiddenflex_bendonly",
            "r2_transformer_hiddenflex_bendonly",
            "cv_r2_bendflex",
            "r2_bendflex",
        ])
    ...
    print("[INFO] 1-mer col:", col_1mer)
    print("[INFO] 1-mer+12flex col:", col_1mer12flex)
    print("[INFO] 2-mer col:", col_2mer)
    print("[INFO] 3-mer col:", col_3mer)
    print("[INFO] y col:", col_model)
    ...
    fig.suptitle(args.figure_title, fontsize=18, y=0.98)
