...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            _, flex_pred = model(x, attention_mask)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            running_train_loss += float(flex_loss.item())
            num_batches += 1
        ...
        print(f"epoch {epoch+1:02d} | train_flex_loss = {avg_train_loss:.4f} | val_flex_loss = {avg_val_loss:.4f}")

    print()
    print("start train flex loss =", train_losses[0])
    print("end train flex loss =", train_losses[-1])
    print("best train flex loss =", min(train_losses))
    print("start val flex loss =", val_losses[0])
    print("end val flex loss =", val_losses[-1])
    print("best val flex loss =", min(val_losses))
    print("flex-only train/val epoch loop complete")
